...
Keep the current dark "kinetic glass" look. Replace every placeholder with your actual details from the uploaded CV and the six Google/Coursera certificates.
...
**CV download** — your uploaded resume PDF becomes the download, so the button gives employers your real document.

## Technical notes

- Content edits live in `src/routes/index.tsx`; page title/description in `src/routes/index.tsx` head() updated to the finance-admin positioning.
- Uploaded resume served via a Lovable asset pointer; the previously generated placeholder CV PDF is removed.
- Certificate PDFs are also published as verifiable downloads linked from the certifications cards.
